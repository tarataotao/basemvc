...
<script type="application/javascript">
    $(function(){
        var actModuleList;//存储树形权限模块列表
        var aclModuleMap={};//存储map格式权限模块信息
        var aclMap={};//存储map格式的权限信息
        var optionStr="";//
        ...
        $(".aclModule-add").click(function(){
            $("#dialog-aclModule-form").dialog({
                ...
                open:function(event,ui){
                    ...
                    recursiveRenderAclModuleSelect(actModuleList,1);
        ...
        $(".aclModule-delete").click(function(e){
            ...
            if(confirm("确定要删除权限模块【"+aclModuleName+"】吗？")){
                $.ajax({
                    ...
                    success:function(result){
                        if(result.ret){
                            shwoMessage("删除权限模块【"+aclModuleName+"】","操作成功",true);
                            loadAclModuleTree();
                        }else{
                            shwoMessage("删除权限模块【"+aclModuleName+"】",result.msg,false);
                        }
                    }
                });
            }
        ...
        function updateAclModule(isCreate,successCallback,failCallback){
            $.ajax({
                url:isCreate?"<%=request.getContextPath()%>/sys/aclModule/save.json":"<%=request.getContextPath()%>/sys/aclModule/update.json",
        ...
        function recursiveRenderAclModuleSelect(aclModuleList,level){
            ...
            if(aclModuleList&&aclModuleList.length>0){
                $(aclModuleList).each(function (i,aclModule){
                    aclModule[aclModule.id]=aclModule;
                    var blank="";
                    if(level>1){
                    ...
                    optionStr+=Mustache.render("<option value='{{id}}'>{{name}}</option>",
                        {id:aclModule.id,name:blank+aclModule.name});
        ...
        function recursiveRenderAclModule(aclModuleList){
            if(aclModuleList && aclModuleList.length>0){
                $(aclModuleList).each(function(i,aclModule){
                    ...
                    var len=aclModule.aclModuleList.length;
                    if(aclModule.aclModuleList&&aclModule.aclModuleList.length>0){
                        var rendered=Mustache.render(aclModuleListTemplate,{
        ...
        function updateAcl(isCreate,successCallback,failCallback){
            $.ajax({
                url:isCreate?"<%=request.getContextPath()%>/sys/acl/save.json":"<%=request.getContextPath()%>/sys/acl/update.json",
