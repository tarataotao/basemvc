...
<script type="text/javascript">
    $(function(){
            ...
            if(aclModuleList && aclModuleList.length>0){
                $(aclModuleList).each(function(i,aclModule){
                    ...
                    if((aclModule.aclModuleList && aclModule.aclModuleList.length>0)||
                        (aclModule.aclList && aclModule.aclList.length)){
                        ...
                        while(hasChecked && tempAclModule){
                            ...
                            tempAclModule=nodeMap[tempAclModule.pId]; //返回上一层
                        }
                    }
                    recrusivePrepareTreeData(aclModule.aclModuleList);//继续递归
                });
            }
        }
        ...
        $(".saveRoleAcl").click(function(e){
            e.preventDefault();
            if(lastRoleId==-1){
                showMessage("保存角色与权限点的关系","请先在左侧选择角色",false);
                return;
            }
            $.ajax({
        ...
        function updateRole(isCreate,successCallback,failCallback){
            $.ajax({
                url:isCreate?"<%=request.getContextPath()%>/sys/role/save.json":"<%=request.getContextPath()%>/sys/role/update.json",
        ...
        $("#roleTab a[data-toggle='tab']").on("show.bs.tab",function(e){
            if(lastRoleId==-1){
                showMessage("加载角色关系","请在左侧选择操作的角色",false);
                return;
            }
